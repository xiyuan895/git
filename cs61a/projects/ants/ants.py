...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self) -> Bee | None:
        ...
        if  self.place.bees:
            ...
            new_place_index = self.place
            while new_place_index and not new_place_index.is_hive:
                if new_place_index.bees:
                    if self.lower_bound <= distance <= self.upper_bound:
                        return random_bee(new_place_index.bees)
                new_place_index = new_place_index.entrance
                distance += 1
            return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount: float):
        ...
        """
        # BEGIN Problem 5
        bee_new = self.place.bees[:]
        kill = 0
        if self.health > amount:
            kill = amount
        else:
            kill = amount + self.damage
        for bees in bee_new:
            bees.reduce_health(kill)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate: GameState):
        ...
        # BEGIN Problem 12
        super().action(gamestate)
        currenct_place = self.place.exit
        while currenct_place:
            if currenct_place.ant:
                if not currenct_place.ant.is_double:
                        currenct_place.ant.is_double = True
                        currenct_place.ant.double()
                if currenct_place.ant.is_container:
                    if not currenct_place.ant.ant_contained.is_double:
                        currenct_place.ant.ant_contained.is_double = True
                        currenct_place.ant.ant_contained.double()
            currenct_place = currenct_place.exit  
